...
- [0257 二叉树的所有路径](../../problems/二叉树/0257%20二叉树的所有路径.md) 🟢（DFS + 回溯入门）
- [0112 路径总和](../../problems/二叉树/0112%20路径总和.md) 🟢（策略 A：纯 DFS + 减法 + 返回值）
- [0113 路径总和 II](../../problems/二叉树/0113%20路径总和%20II.md) 🟡（257 + 112 合体：path 回溯 + sum 回溯 + 深拷贝收集）
- [0437 路径总和 III](../../problems/二叉树/0437%20路径总和%20III.md) 🟡（嵌套 DFS O(n²)；前缀和 O(n) 待补）

## 🎓 tree_path 毕业（按 CLAUDE.md「Pattern 推进策略」）

4 题覆盖 3 种「质」：

| 质 | 题 |
|---|---|
| 收集所有路径（回溯 + 深拷贝） | 0257、0113 |
| 存在性判定（DFS + 返回值） | 0112 |
| 任意起止 / 嵌套 DFS / 前缀和 | 0437 |

模板默写无压力 → **tree_path 毕业**（前缀和优化解待补，不影响"质"覆盖）。
